...
function readFiles(dirPath, filenames) {
	...
	getLogger().info('Searching for language input files...');
		...
		.then(function(content) {
			...
			if(content.length > 0) {
				try {
					...
					getLogger().info('Syntax error parsing JSON in "%s"', filename);
					throw e;
				}
			}
			return {
...
function extract(files, fallback) {
	...
	getLogger().info('\tBase languages: ' + langStr);
	getLogger().info('\tRegion overrides: ' + regionStr);
...
function combine(input, fallback) {
	...
	getLogger().info('Merging base languages with fallback [%s]...', fallback);
	...
	getLogger().info('Merging region overrides with base languages...');
...
function writeFiles(output, input) {
	return fs
		...
		.then(function(stat) {
			...
			getLogger().info('Writing to output directory...');
